# Yunlong Lu created at 2022/11/25, advised by Wenxin Li, PKU AI Lab
...
class Learner(Process):
    ...
    def run(self):
        ...
        cur_time = time.time()
        iterations = 0
        max_iterations = self.config.get('max_iterations', 100000)  # 从配置读取，默认10万轮
        save_interval = self.config.get('save_interval', 100)  # 每100轮保存一次
        ...
        try:
            ...
            # 捕获 Ctrl+C，保存最终模型
            path = self.config['ckpt_save_path'] + 'model_final.pt'
            torch.save(model.state_dict(), path)
            print('Training interrupted. Final model saved to', path)
            raise